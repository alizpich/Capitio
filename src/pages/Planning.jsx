...
export default function Planning() {
  ...
  async function save() {
    ...
    setSaving(true)
    const { error } = await supabase.from('creneaux').insert(form)
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginBottom: 16 }}>
        <div onClick={() => setFiltreType('tous')}
          style={{ padding: '6px 14px', borderRadius: 99, fontSize: '0.78rem', fontWeight: 500, cursor: 'pointer', border: '1.5px solid', transition: 'all 0.15s', borderColor: filtreType === 'tous' ? '#0B1A3E' : '#E2E8F0', background: filtreType === 'tous' ? '#0B1A3E' : '#fff', color: filtreType === 'tous' ? '#fff' : '#6B7A8D' }}>
          Tous
        </div>
        {TYPES.map(t => (
          <div key={t} onClick={() => setFiltreType(t)}
            style={{
              padding: '6px 14px', borderRadius: 99, fontSize: '0.78rem', fontWeight: 500, cursor: 'pointer',
              border: `1.5px solid ${filtreType === t ? TYPE_COLORS[t] : '#E2E8F0'}`,
              background: filtreType === t ? TYPE_BG[t] : '#fff',
              color: filtreType === t ? TYPE_COLORS[t] : '#6B7A8D',
      ...
      {loading ? (
        ...
        <div key={day} style={{ marginBottom: 22 }}>
          <div style={{ fontSize: '0.8rem', fontWeight: 700, color: '#9BA8B5', textTransform: 'capitalize', marginBottom: 10, padding: '0 4px', letterSpacing: '0.04em' }}>
            {day}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {items.map(c => {
              ...
              return (
                <div key={c.id} style={{
                  background: '#fff', border: `1px solid ${conflict ? 'rgba(229,62,62,0.35)' : '#E2E8F0'}`,
                  borderRadius: 12, padding: '12px 16px',
                  display: 'flex', alignItems: 'center', gap: 14,
                  borderLeft: `4px solid ${TYPE_COLORS[c.type]}`,
                }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '0.88rem', fontWeight: 600, color: '#0B1A3E', marginBottom: 3 }}>
                      {c.titre}
                    </div>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      <Badge variant={c.type === 'match' ? 'green' : c.type === 'reunion' ? 'purple' : c.type === 'evenement' ? 'orange' : 'blue'}>
                        {TYPE_LABELS[c.type]}
                      </Badge>
                  ...
                  <button onClick={() => deleteCreneau(c.id)}
                    style={{ width: 26, height: 26, borderRadius: 6, border: '1px solid #E2E8F0', background: '#fff', cursor: 'pointer', fontSize: '0.72rem', color: '#9BA8B5', flexShrink: 0 }}
      ...
      >
        <Field label="Titre du créneau">
          <Input value={form.titre} onChange={set('titre')} required placeholder="ex. Entraînement Senior" />
        </Field>
        <Field label="Type">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <Field label="Début">
            <Input type="datetime-local" value={form.date_debut} onChange={set('date_debut')} />
          </Field>
          <Field label="Fin">
            <Input type="datetime-local" value={form.date_fin} onChange={set('date_fin')} />
          </Field>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <Field label="Équipe">
            <Input value={form.equipe} onChange={set('equipe')} placeholder="ex. Senior" />
          </Field>
          <Field label="Terrain / Lieu">
            <Input value={form.terrain} onChange={set('terrain')} placeholder="ex. Terrain A" />
          </Field>
        </div>
      </Modal>
    </PageShell>
